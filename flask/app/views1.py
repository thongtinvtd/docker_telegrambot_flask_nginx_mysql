...
def postLogin():
    try:
        ...
        if _username == 'admin':
            ...
            rows_pass = config.sqlselect(sql_checkpass)
            if rows_pass:
...
def postChangepass():
    try:
        ...
        rows_oldpass = config.sqlselect(sql_oldpass)
        if rows_oldpass : 
...
def change_u() :
    ...
    for i in usertable:
        ...
        sql1 = "UPDATE alert_list SET permission='{}',comment ='{}',expiration='{}' WHERE user_id={} ;".format(user_permi, user_comm,user_datetime,i)
        curs.execute(sql1)
        conn.commit()
    return redirect(url_for('dashboard'))
#--------------------------------------------------------------------
# handle commad change parameters on messager to user
...
def change_p() :
    ...
    for i in paraTable:
        if i[1] in l_check_p:
            sql1 = "UPDATE para_manager SET on_messager = 'Yes' WHERE parameter = '{}' ;".format(i[1])
            print(sql1)
            curs.execute(sql1)
            conn.commit()
        else:
            sql1 = "UPDATE para_manager SET on_messager = 'No' WHERE parameter = '{}' ;".format(i[1])
            print(sql1)
            curs.execute(sql1)
            conn.commit()
...
def change_t() :
    ...
    if config.is_number(timecycle) and config.is_number(timereq) :
        if  float(timecycle) > 0 and float(timereq) > 0  :
            sql3 = "UPDATE para_manager SET value = {} WHERE parameter = 'Time_cycle';".format(timecycle)
            sql4 = "UPDATE para_manager SET value = {} WHERE parameter = 'Time_req';".format(timereq)
            print(sql3)
            curs.execute(sql3)
            conn.commit()
            curs.execute(sql4)
...
def dashboard():
    if 'username' in session:
        ...
        l_varReq = []
        sql0 = 'SELECT parameter,on_messager,value FROM para_manager;'
